...
impl EventSender {
    pub fn new(broadcaster: Broadcaster) -> Self {
        ...
        let handler_loop = spawn(async move {
            if let Err(e) = EventSender::handler_loop(receiver, broadcaster).await {
                log::error!("{}", e);
            }
        });
        Self {
    ...
    async fn handler_loop(
        ...
        let mut mouse_position = CanvasCoordinate::default();
                ...
                FutureType::Broadcaster(msg) => {
                    match msg {
                        BroadcastMsg::RequestMousePosition => {
                            broadcast_cell.send(BroadcastMsg::MousePosition(mouse_position))?;
                        }
                    ...
                    match input {
                        Input::Button(button) => match button.button {
                            ...
                            input::Button::Mouse(mouse) => {
                                ...
                                {
                                    broadcast_cell
                                        .send(BroadcastMsg::MouseClick(mouse_position))?;
                                }
                            }
                            _ => {}
                        },
